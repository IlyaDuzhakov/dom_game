...
describe("getRandomIndex recursion", () => {
  //используется для группировки связанных тестовых случаев, типа как див в HTML
  beforeEach(() => {
    //beforeEach - позволяет выполнить определенный код перед каждым тестом в рамках тестового файла или блока describe.
    resetLastIndex();
  ...
  -(
    ...
    test("calls itself recursively when number === lastIndex", () => {
      ...
      jest.spyOn(Math, "random").mockImplementation(() => {
        ...
        callCount++;
        if (callCount === 1)
          return 5 / 16; // Math.floor = 5 (совпадение)
